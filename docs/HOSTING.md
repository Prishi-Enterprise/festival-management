...
Updated 23 September 2026. The current configuration below supersedes the historical release notes.

## Current environments and release workflow

| Git branch | Website | Supabase |
| --- | --- | --- |
| `develop` | `https://dev.festivals.prishi.in` | Festival Management Dev, Tokyo `pokeislmfmfuqgfkxmzp` |
| `main` | `https://festivals.prishi.in` | Festival Management, Mumbai `gfxtoblbiravcdwgerxw` |

One public repository, `Prishi-Enterprise/festival-management`, and one Vercel project, `festival-management` in `prishi-ai`, serve both environments. Push development work to `develop`; Vercel builds it as Preview. After dev acceptance and any required production database migrations, merge the approved changes into `main` for a fresh Production build. Never promote a Preview build containing dev database values to production.

### Required Gitflow policy

1. Branch feature/fix work from current `develop` and integrate it into `develop` first. This applies to every change, including documentation, benchmarks and urgent fixes.
2. Deploy and validate dev. Run tests, lint, typecheck and a production build before release; apply database migrations only to the approved environment.
3. With user release approval, open a release PR with **base `main`, head `develop`**. Nothing is committed/pushed directly to `main`; feature/fix branches never merge directly into it. GitHub requires a PR even for administrators and blocks force pushes and deletion. The head-branch and dev-acceptance requirements are working policy; branch protection alone does not validate them.
4. Merge the release PR with a merge commit, preserving shared history. Fast-forward `develop` to the resulting `main` commit and push `develop`; update local `main` by fast-forward as well. Avoid squash/rebase release merges, which cause the long-lived branches to diverge.
5. Verify the intended deployment and record any deliberately deferred production release. If the user approves a documentation/benchmark-only sync but explicitly defers production deployment, cancel the automatic production build and verify the production alias still serves the preceding release. The next approved application release includes the accumulated changes and builds normally.

This policy supersedes earlier direct-to-main release practices. A merge into `main` normally triggers Vercel automatically; a skipped production release must be handled explicitly, never assumed from the changed filenames.

Preview Supabase variables and `NEXT_PUBLIC_APP_ENV=dev` apply to previews. `APP_URL=https://dev.festivals.prishi.in` is scoped specifically to Preview branch `develop`. Production retains its Mumbai variables and `APP_URL=https://festivals.prishi.in`. Other temporary preview branches use their own generated hostname. RSVP and pass links generated by develop use the canonical dev address.

GoDaddy CNAME `dev.festivals` points to `34da08b98725dead.vercel-dns-017.com`, and Vercel associates the domain specifically with `develop`. DNS verification must finish before the new address is usable. Dev Supabase Auth Site URL is `https://dev.festivals.prishi.in`, with exact redirect `https://dev.festivals.prishi.in/auth/callback`. Production Auth URLs remain on `festivals.prishi.in`. No SMTP/domain changes are needed in Resend.

The former `codex/launch-candidate` branch is superseded by `develop`. Development data and production data remain separate; changing Git branches does not copy database records. Finance permissions and QR attendance remain dev-only until their production release is approved.

Local development still uses Supabase on Colima at `http://localhost:3000`. All environments use session-based database authorization, never a service-role key in the app.

## Historical initial launch — 21 September 2026

The notes below record earlier rollout stages; their old domain/branch references are historical.
